...
/* =====================================================
   INTERNAL: RESOLVE BUYER_ID (BACKWARD COMPATIBLE)
   - HỖ TRỢ ĐƠN CŨ
===================================================== */
async function resolveBuyerIdByPiUid(
  piUid: string
): Promise<string | null> {
  ...
  const users = (await res.json()) as Array<{ pi_uid: string }>;
  return users[0]?.pi_uid ?? null;
}

/* =====================================================
   GET ORDERS BY BUYER (CUSTOMER)
   - HIỂN THỊ ĐƯỢC ĐƠN CŨ + ĐƠN MỚI
===================================================== */
export async function getOrdersByBuyerSafe(
  buyerPiUid: string
): Promise<OrderRecord[]> {
  const buyerId = await resolveBuyerIdByPiUid(buyerPiUid);
  if (!buyerId) return [];
...
/* =====================================================
   ...
   - KHÔNG PHỤ THUỘC USERS SAU KHI TẠO
===================================================== */
